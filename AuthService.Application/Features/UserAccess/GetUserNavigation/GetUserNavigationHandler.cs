...
public class GetUserNavigationHandler : IRequestHandler<GetUserNavigationQuery, UserNavigationDto>
{
    ...
    public GetUserNavigationHandler(UserManager<ApplicationUser> userManager, IQueryDbContext context)
    {
    ...
    public async Task<UserNavigationDto> Handle(GetUserNavigationQuery request, CancellationToken cancellationToken)
    {
        ...
        bool isSuperAdmin = userRoleNames.Contains("SuperAdmin", StringComparer.OrdinalIgnoreCase);
        ...
        // Get user's department (if any) - use Set<T> for entities not in interface
        ...
        var userPermissionIds = await _context.RolePermissionMappings
        ...
        if (isSuperAdmin)
        {
            ...
            accessiblePages = await _context.Pages
                .Where(p => p.IsActive)
                .ToListAsync(cancellationToken);
        }
        else
        {
            ...
            var pageIdsFromPermissions = await _context.PagePermissionMappings
        ...
        var pageFeatureMappings = await _context.PageFeatureMappings
